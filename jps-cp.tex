...
The CS inner shield system is placed in a harsh environment of photon and neutron fluxes of $F_\gamma \sim \mathcal{O}(10^9)~\unit{\per\square\cm\per\second}$ and $F_n \sim \mathcal{O}(10^{10})~\unit{\per\square\cm\per\second}$, respectively, in a high and graded magnetic field ranging \qtyrange{2.5}{4.9}{\tesla}\footnote{Actual operational field strength is lower by a factor \qtyrange{10}{20}{\percent} than the design peak value of \SI{4.9}{T} because of the ground fault incident happened in the CS coil circuit, but the CS inner shield was designed based on the design value.}, and in a vacuum volume of $\mathcal{O}(10^{-4})$~\unit{\Pa}\footnote{In Phase-I, the vacuum level would be lower, $\mathcal{O}(10^{-2})$~\unit{\Pa}, because such high vacuum is not necessary for the graphite target.  However, the CS inner shield is designed to achieve the high vacuum level considering the future upgradability to use a radiation cooling tungsten target \cite{CM46}.}. 
...
\paragraph{Material}
Forged material of non-magnetic SS304 with a density of \SI{7.6}{\gram\per\cubic\cm} or higher shall be used. Demagnetization treatment such as solution heat treatment shall be performed. Mill sheets for the main materials shall be provided.
...
\end{figure}
 The first four SS304 modules and upstream and downstream endplates were produced by a Japanese company (KOMIYAMA ELECTRON Corp.) in FY2024 (see \figref{fig:CSInnerShield_2024production}).
...
Hot rolled stainless steel plates (JIS G 4304 SUS304-HP) with a heat treatment of   $\SI{1080}{\degreeCelsius} \times \SI{3}{\minute}$ (water cooled) were used for the upstream and downstream end-plates.
 ...
 To achieved the specified flatness at the downstream end-plate surface, the length of each module was adjusted with shim plates.
