...
\maketitle
\begin{abstract}
The COMET experiment, which searches for the lepton-flavor-violating muon-to-electron conversion process, is under preparation in the J-PARC Hadron Experimental Facility. To provide a high-intensity muon beam to the experiment,  pions produced from the collision of \SI{8}{\GeV} protons from the Main Ring with a graphite target are collected using a superconducting pion capture solenoid. It is necessary to install a massive radiation shield inside the capture solenoid to protect the coils from the quench as well as radiation damage.
...
Please note that unless specified ``upstream/downstream'' indicate those with respect to the primary beam for the CS inner shield system. It could be opposite for other systems (such as secondary beam line and detectors).


\subsection{Overview of the shield design}
In the early stage of the design of the experiment as in Refs.~\cite{comet,inner-shield}, a tungsten-based alloy was considered as the main material of the shield.
...
manufactured individually and then they are assembled with the upstream and downstream end-plates.
...
In the original design, the shape was identical for all modules with a tapered structure both in the upstream and downstream (for the proton beam) of the target to pass the bent beam.
...
\paragraph{Material}
Forged material of non-magnetic SS304 with a density of \SI{7.6}{\gram\per\cubic\cm}or higher\footnote{The actual density of the material used in the 2024 production was \SI{7.84}{\gram\per\cubic\cm}.}. Demagnetization treatment such as solution heat treatment shall be performed. Mill sheets for the main materials shall be provided.
...
\paragraph{Blackbody plate}
 To absorb the radiant heat of the target, blackbody plates with blackbody treatment (equivalent to RAYDENT LSL-BL) shall be bolt-connected to the part corresponding to the inner cylinder of the SS304 shield ASSY. The surface roughness of the contact faces shall be Ra3.2 or better for good thermal contact. 

\paragraph{Thermal shield}
\SI{0.5}{\mm} thick SS304 thermal isolation shields shall be bolt-connected to the surface corresponding to the outer cylinder of each SS304 shield to reflect back the radiant heat of the main body. The thermal shields shall be electrochemical polished.  %Multiple pieces are acceptable as long as they cover the entire outer cylinder surface. 
...
Details of SS304 and OFC modules are different. The OFC module shown in Fig.~\ref{fig:b} has a more complex design due to its weaker mechanical strength and higher electrical conductivity.  %Here, the detailed design of more complicated OFC module is described.
...
It also serves as the interface for a target system with a precise position reference. 
The target system is designed to be exchangeable independently of the CS inner shield.
...
\subsection{Detailed design of the downstream flange ASSY}
...
To check the mechanical strength of the whole system, we performed a structural analysis, and the original design turned out to be not enough against the horizontal load from the large-bore pillow seal and the seismic load.
...
\end{figure}
Figure~\ref{fig:CSInnerShield_downstreamFlange} shows the updated design. The thickness of the mechanical flange is increased from \SI{70}{\mm} to \SI{120}{\mm} and ribs are added.
...
The neutron fluence is also evaluated as shown in \figref{fig:CSImpact}(b). Here neutrons \SI{>0.1}{\MeV} are counted because they are known to damage the coils \cite{pcs-irradiation}.  The fluence in 150 days beam exposure is $\Phi_n \sim 1\times10^{16}~\unit{\per\square\cm}$ at the maximum position.
