...
In its first stage, COMET Phase-I, we aim for the single-event sensitivity of \num{3e-15} and need \SI{>1e9}{\per\second} negative muons to be stopped in aluminum targets.
To make such a high-intensity negative muon beam, we use a \SI{490}{\mm} long graphite target hit by the primary proton beam of \SI{3.2}{\kW}.
...
\subsection{Conceptual design of OFC module}
...
Air cooling is expected in the Phase-I operation, but it is designed so that water cooling is also possible. If it is necessary, for instance, in a higher beam power operation in future, we can switch to the water cooling from outside of the beam room.
...
\begin{figure}[tbp]
  ...
  \caption{Impact on the superconducting coils for an exposure to \SI{3.2}{\kilo\watt} beam with a 490-mm long graphite target. The horizontal axes show the different coil regions, from the upstream side of CS (with respect to the secondary beam) to the bridge solenoid. The CS coils are subdivided into 12 sections in the azimuth direction.}
  \label{fig:CSImpact}
\end{figure}
...
The total heat load is  \SI{\sim 13.6}{\watt} (a raw value not including a safety factor).
...
\begin{figure}[tbp]
  ...
  \caption{Comparison of the impact on the superconducting coils with and without the CS inner shield for an exposure to \SI{3.2}{\kilo\watt} beam with a 490-mm long graphite target. }
