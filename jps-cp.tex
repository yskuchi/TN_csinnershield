...
\fancyhead[L]{COMET-doc-183-v1}   % 左
...
\maketitle
\begin{abstract}
To provide a high-intensity muon beam to the COMET experiment,  pions produced from the collision of \SI{8}{\GeV} protons from the J-PARC Main Ring with a graphite target are collected using a superconducting pion capture solenoid (CS). It is necessary to install a massive radiation shield inside the CS to protect the coils from the quench and radiation damage.
...
Figure~\ref{fig:overview} shows the schematic view of the pion production system in the COMET beam room, which locates in the basement of the Hadron South Experimental Building in J-PARC.

Secondary particles from the collision of the proton beam with the production target induce heat in the cold mass of the CS and subsequently a quench of the superconductor. Exposure to radiation also leads to degradation of thermal conductivity in the conduction-cooled superconducting coils, which limits the continuous operation of the CS system \cite{inner-shield}. Therefore, it is necessary to install a massive radiation shield inside the warm bore of CS.

In this note, we describe the technical and engineering design of the CS inner shield system. 
\textcolor{red}{Note that some information and drawings are confidential}. The visibility of this note is restricted to the COMET collaboration. Figures labelled with \textcolor{red}{ ``CONFIDENTIAL''} must not be open to outside the collaboration.

\begin{figure}[!b]
\includegraphics[width=\textwidth]{fig/Overview.pdf}
\caption{Schematics of the COMET beam room and the pion production system. %It is located in the basement of the Hadron South Experimental Building in J-PARC. 
Trajectories in red, green, and light blue are protons, negative pions, and negative muons, respectively. One hundred \SI{8}{\GeV} protons are injected into the production target in this figure. The other particles are omitted.}
\label{fig:overview}
\end{figure}

\section{Requirements for the CS inner shield}
The CS inner shield system is placed in a harsh environment of photon and neutron fluxes of $F_\gamma \sim \mathcal{O}(10^9)~\unit{\per\square\cm\per\second}$ and $F_n \sim \mathcal{O}(10^{10})~\unit{\per\square\cm\per\second}$, respectively, in a high and graded magnetic field ranging \qtyrange{2.5}{4.9}{\tesla}\footnote{Actual operational field strength is lower by a factor \qtyrange{10}{20}{\percent} than the design values because of the ground fault incident happened in the CS coil circuit, but the CS inner shield was designed based on the design value.}, and in a vacuum volume of $\mathcal{O}(10^{-4})$~\unit{\Pa}\footnote{In Phase-I, the vacuum level would be lower, $\mathcal{O}(10^{-2})$~\unit{\Pa}, because such high vacuum is not necessary for the graphite target.  However, the CS inner shield is designed to achieve the high vacuum level considering the future upgradability to use a radiation cooling tungsten target \cite{CM46}.}. 
...
In the early design stage of the experiment as in Refs.~\cite{inner-shield,comet}, a tungsten-based alloy was considered as the main material of the shield.
...
The heat load on the shield is expected to be \SI{1}{\kW} in total and \SI{160}{\watt} in the module at maximum
...
%See \secref{sec:production} for the actual values.
\paragraph{Material}
Forged material of non-magnetic SS304 (SUS304 or 06Cr19Ni10) with a density of \SI{7.6}{\gram\per\cubic\cm} or higher shall be used. Demagnetization treatment such as solution heat treatment shall be performed. Mill sheets for the main materials shall be provided.
...
\item Pressure test: apply \SI{1.5}{\mega\pascal} to the cooling pipes using water and confirm no pressure drop within 1 hour.
\end{enumerate}

\paragraph{Temperature measurement}
The temperature of each SS304 shield will be measured with two E-type thermocouples. Taps and grooves for fixing and wiring the thermocouples shall be made. 
...
If the adjacent module is a OFC one, fasten the adjacent surface with SS304 connection plates.
...
%Details of OFC modules are different from the SS304 one. 
The OFC module shown in Fig.~\ref{fig:b} has a more complex design due to its weaker mechanical strength and higher electrical conductivity.  %Here, the detailed design of more complicated OFC module is described.
...
\begin{figure}[tbp]
  ...
  \caption{Impact on the superconducting coils for an exposure to \SI{3.2}{\kilo\watt} beam with a 490-mm long graphite target and \SI{4.4}{\tesla} CS field. The horizontal axes show the different coil regions, from the beam-dump side of CS to the bridge solenoid. The CS coils are subdivided into 12 sections in the azimuthal direction.
In this calculation, a 1-mm clearance is assumed between adjacent modules of the CS inner shield.}
  \label{fig:CSImpact}
\end{figure}
The impact of the irradiation on the superconducting coils was studied with PHITS (version 3.35.1) \cite{phits} .
\Figref{fig:CSImpact}(a) shows the energy deposit density in the coils for different regions of the coils when the CS inner shield is installed.
The peaks correspond to the 8 and 9 o'clock positions, where the OFC modules locate.
The total heat load is calculated to be  \SI{\sim 13.6}{\watt} (not including a safety factor) by summing the deposit density with weights from the design values of the coils since the implementation of coil geometry in the PHITS simulation is not perfect. 
...
\section{Installation and extraction method}
We designed a method for installation and extraction to minimize the exposure of workers to the residual dose.
The residual dose after the Phase-I operation was calculated and evaluated with a PHITS  simulation.
...
A  \SI{100}{\mm} thick iron shielding wall is placed between the mechanical flange and the lifting jig so that workers can approach the flange. The connection/disconnection at the feedthroughs and closing/opening the aluminum flange will be done over the shielding wall.
...
\subsection{Production in 2024 (Japan)}
\begin{figure}[tbp]
\centering
\includegraphics[width=1\linewidth]{fig/CSInnerShield_2024production.pdf}
\caption{Assembly of the 2024 production with four SS304 modules and upstream and downstream end-plates. It was delivered, packaged in a nitrogen-purged plastic bag, to KEK Tsukuba campus on 14 March, 2025.}
\label{fig:CSInnerShield_2024production}
\end{figure}
 The first four SS304 modules and upstream and downstream endplates were manufactured by a Japanese company (KOMIYAMA ELECTRON Corp.) in FY2024 (see \figref{fig:CSInnerShield_2024production}).
A heat treatment of $\SI{1050}{\degreeCelsius} \times \SI{11}{\hour}$ (water cooled) was applied to the raw material (JIS G 4303 SUS304) for the main shield body. 
Rectangular samples were submitted from each material. The density was measured from these samples to be \SI{7.84}{\gram\per\cubic\cm}.
Hot rolled stainless steel plates (JIS G 4304 SUS304-HP) with a heat treatment of   $\SI{1080}{\degreeCelsius} \times \SI{3}{\minute}$ (water cooled) were used for the upstream and downstream end-plates.

 Following discussions with the manufacturer, a crank structure was introduced at the interface between the upper three modules, and the clearance between them was set to \SI{1.5}{\mm} as shown in \figref{fig:CSInnerShield_2024crank}.
 The machining accuracy was quite high; most of the parameters were within \SI{20}{\micro\metre}.
 To achieve the specified flatness at the downstream end-plate surface, the length of each module was adjusted with shim plates.
  All the specifications were satisfied.
\begin{figure}[tbp]
\centering
\includegraphics[width=1\linewidth]{fig/CSInnerShield_2024crank.pdf}
\caption{ \textcolor{red}{CONFIDENTIAL} Details of the crank structure.}
\label{fig:CSInnerShield_2024crank}
\end{figure}
...
The production and construction of the system began in 2024.
...
\begin{description}[itemsep=3pt, parsep=0pt]
  \item[2024 Jul.] Tendering for the first four SS304 modules, upstream and downstream endplates (Japan).
...
\caption{ \textcolor{red}{CONFIDENTIAL} Assembly drawing of SS304 module.}
\label{fig:SS304ASSY}
\end{figure}
...
\caption{ \textcolor{red}{CONFIDENTIAL} Part drawing of SS304 module main body.}
\label{fig:SS304Part}
\end{figure}
...
\caption{ \textcolor{red}{CONFIDENTIAL} Assembly drawing of OFC module.}
\label{fig:OFCASSY}
\end{figure}
...
\caption{ \textcolor{red}{CONFIDENTIAL} Part drawing of OFC module left body.}
\label{fig:OFCPart}
\end{figure}
...
\caption{ \textcolor{red}{CONFIDENTIAL} Part drawing of the upstream endplate.}
...
\caption{ \textcolor{red}{CONFIDENTIAL} Part drawing of the downstream endplate.}
...
\caption{ \textcolor{red}{CONFIDENTIAL} Assembly drawing of mechanical support.}
...
\caption{ \textcolor{red}{CONFIDENTIAL} Part drawing of the mechanical flange.}
...
\caption{ \textcolor{red}{CONFIDENTIAL} Part drawing of the support beams and the support flange.}
...
\caption{ \textcolor{red}{CONFIDENTIAL} Assembly drawing of vacuum flange system.}
...
\caption{ \textcolor{red}{CONFIDENTIAL} Part drawing of the aluminum flange.}
...
\bibitem{CM46} S. Makimura, presented in CM46 (2025), \url{https://kds.kek.jp/event/55658/contributions/294871/attachments/192087/261334/J-PARC_Target2.pdf}.
